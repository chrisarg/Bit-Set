...
$ffi->type('record(Bit::Set::DB::SETOP_COUNT_OPTS)' => 'SETOP_COUNT_OPTS_t'); ## LMM didn't generate the record token in the definition
...
my %functions = (
    ...
    BitDB_new => {
        ...
        check => sub {
            ...
            die "BitDB_new: length must be >= 0 and <= INT_MAX" if $length < 0 || $length > 2147483647;
            die "BitDB_new: num_of_bitsets must be >= 0 and <= INT_MAX" if $num_of_bitsets < 0 || $num_of_bitsets > 2147483647;
        }
    },
    BitDB_free => {
        args => ['Bit_DB_T*'],
    ...
    BitDB_inter_count_store_cpu => {
        ...
        check => sub {
            ...
            die "BitDB_inter_count_store_cpu: containers cannot be NULL" if !defined $bit || !defined $bits;
            die "BitDB_inter_count_store_cpu: buffer cannot be NULL" if !defined $buffer;
        }
    },
    BitDB_union_count_store_cpu => {
        ...
        check => sub {
            my ($bit, $bits, $buffer, $opts) = @_;
            die "BitDB_union_count_store_cpu: containers cannot be NULL" if !defined $bit || !defined $bits;
            die "BitDB_union_count_store_cpu: buffer cannot be NULL" if !defined $buffer;
        }
    },
    BitDB_diff_count_store_cpu => {
        ...
        check => sub {
            my ($bit, $bits, $buffer, $opts) = @_;
            die "BitDB_diff_count_store_cpu: containers cannot be NULL" if !defined $bit || !defined $bits;
            die "BitDB_diff_count_store_cpu: buffer cannot be NULL" if !defined $buffer;
        }
    },
    BitDB_minus_count_store_cpu => {
        ...
        check => sub {
            my ($bit, $bits, $buffer, $opts) = @_;
            die "BitDB_minus_count_store_cpu: containers cannot be NULL" if !defined $bit || !defined $bits;
            die "BitDB_minus_count_store_cpu: buffer cannot be NULL" if !defined $buffer;
        }
    },
    ...
    BitDB_inter_count_store_gpu => {
        ...
        check => sub {
            my ($bit, $bits, $buffer, $opts) = @_;
            die "BitDB_inter_count_store_gpu: containers cannot be NULL" if !defined $bit || !defined $bits;
            die "BitDB_inter_count_store_gpu: buffer cannot be NULL" if !defined $buffer;
        }
    },
    BitDB_union_count_store_gpu => {
        ...
        check => sub {
            my ($bit, $bits, $buffer, $opts) = @_;
            die "BitDB_union_count_store_gpu: containers cannot be NULL" if !defined $bit || !defined $bits;
            die "BitDB_union_count_store_gpu: buffer cannot be NULL" if !defined $buffer;
        }
    },
    BitDB_diff_count_store_gpu => {
        ...
        check => sub {
            my ($bit, $bits, $buffer, $opts) = @_;
            die "BitDB_diff_count_store_gpu: containers cannot be NULL" if !defined $bit || !defined $bits;
            die "BitDB_diff_count_store_gpu: buffer cannot be NULL" if !defined $buffer;
        }
    },
    BitDB_minus_count_store_gpu => {
        ...
        check => sub {
            my ($bit, $bits, $buffer, $opts) = @_;
            die "BitDB_minus_count_store_gpu: containers cannot be NULL" if !defined $bit || !defined $bits;
            die "BitDB_minus_count_store_gpu: buffer cannot be NULL" if !defined $buffer;
        }
    },
    ...
    BitDB_inter_count_cpu => {
        ...
        check => sub {
            my ($bit, $bits, $opts) = @_;
            die "BitDB_inter_count_cpu: containers cannot be NULL" if !defined $bit || !defined $bits;
        }
    },
    BitDB_union_count_cpu => {
        ...
        check => sub {
            my ($bit, $bits, $opts) = @_;
            die "BitDB_union_count_cpu: containers cannot be NULL" if !defined $bit || !defined $bits;
        }
    },
    BitDB_diff_count_cpu => {
        ...
        check => sub {
            my ($bit, $bits, $opts) = @_;
            die "BitDB_diff_count_cpu: containers cannot be NULL" if !defined $bit || !defined $bits;
        }
    },
    BitDB_minus_count_cpu => {
        ...
        check => sub {
            my ($bit, $bits, $opts) = @_;
            die "BitDB_minus_count_cpu: containers cannot be NULL" if !defined $bit || !defined $bits;
        }
    },
    ...
    BitDB_inter_count_gpu => {
        ...
        check => sub {
            my ($bit, $bits, $opts) = @_;
            die "BitDB_inter_count_gpu: containers cannot be NULL" if !defined $bit || !defined $bits;
        }
    },
    BitDB_union_count_gpu => {
        ...
        check => sub {
            my ($bit, $bits, $opts) = @_;
            die "BitDB_union_count_gpu: containers cannot be NULL" if !defined $bit || !defined $bits;
        }
    },
    BitDB_diff_count_gpu => {
        ...
        check => sub {
            my ($bit, $bits, $opts) = @_;
            die "BitDB_diff_count_gpu: containers cannot be NULL" if !defined $bit || !defined $bits;
        }
    },
    BitDB_minus_count_gpu => {
        ...
        check => sub {
            my ($bit, $bits, $opts) = @_;
            die "BitDB_minus_count_gpu: containers cannot be NULL" if !defined $bit || !defined $bits;
        }
    },
);
